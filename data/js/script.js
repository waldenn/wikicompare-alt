...
function removeColumn( gid ) {
  ...
  // remove from list in Explore
  window.parent.postMessage({ event_id: 'remove-compare-item', data: { mygid: gid  } }, '*' );
}

function removeAllColumns() {

  console.log( 'remove all columns' );

  // hide table
  $('#table-output').css({ 'display' : 'none' });

  // remove from list in Explore
  window.parent.postMessage({ event_id: 'remove-compare-items', data: {  } }, '*' );
}
...
class Ui {
	...
	static getIdName(wdId, label) {
		return `${($("#setting-ui-displaywdid").prop("checked") || label == null) ? _e(wdId) : label}`;
	}
}
var elements;
var properties;
...
function generateTable(elements) {
		...
		if(Object.keys(e).length > 0) {
			...
			elements.forEach(function(wdId, i) {
				...
				if(typeof e[wdId].aliases != "undefined") {
					...
					cells += `<td class="snak-undefined" data-entity="${wdId}"> <i class="fas fa-times"></i> </td>`;
				}
			});
			...
			whitelistedProperties.forEach(function(property, i) {
              ...
              //console.log( wdDisplayValue(val) );
						...
						row.append(`<td class="snak-undefined" data-entity="${wdId}"> <i class="fas fa-times"></i>  </td>`);
					}
				});
	...
	function generateColumns(elements) {
		...
		$.each(elements, function(colIndex, wdId) {
			var cell = $(`<th scope="col" ${colIndex == 0 ? `class="reference-item" ` : ""} data-entity="${wdId}"> <a href="https://www.wikidata.org/wiki/${wdId}" target="_blank" title="${wdId}">${Ui.getIdLabel(wdId, null)}</a> <span class="buttonRemoveColumn" title="remove this column" onClick="removeColumn(&quot;${wdId}&quot;)">&nbsp;<i class="fas fa-times-circle"></i></span> </th>`);
...
function wdDisplayValue(claim) {
	...
	if(claim.mainsnak.snaktype == "somevalue") {
      ...
      var img_url =  'https://commons.wikimedia.org/wiki/Special:FilePath/' + encodeURIComponent( `${_e(claim.mainsnak.datavalue.value)}` ) + '?width=300px';
			return $(`<a href="https://commons.wikimedia.org/wiki/File:${_e(claim.mainsnak.datavalue.value)}" target="_blank"><img class="thumbnail" src="${ img_url }"></a>`);
